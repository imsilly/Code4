package app.utils
{
	...
	public class CustomScroller extends Sprite
	{
		...
		*   */
		public function CustomScroller (ease:Number = 1, direction:String = "vertical", setDraggerSize:Boolean = false, Width:Number = 530, Height:Number = 450, content:MovieClip = null)
		{
		...
		{
			_mask=drawRect(tWidth,tHeight,0x000000,_mask);
			
			if(!_mask.stage)addChild(_mask);
		}
		private function setScrollBar(sbWidth:Number = 5, tWidth:Number = 530, tHeigth:Number = 450):void
		{
			sbWidth = 10;
			_dragger=drawRect(sbWidth,sbWidth,0x999999,_dragger);
			_draggerBg = drawRect(sbWidth,tHeigth,0x333333,_draggerBg);
			_dragger.x = _draggerBg.x = tWidth - 20;
			_orgY=_dragger.y;
			_dragger.buttonMode = true;
			
			if(!_draggerBg.stage)this.addChild(_draggerBg);
			if(!_dragger.stage)this.addChild(_dragger);
		...
		private function drawRect(w:Number, h:Number, col:uint,s:Sprite=null):Sprite
		{
		...
		/*private function drawRoundRect(w:Number, h:Number, col:uint,s:Sprite=null):Sprite
		{
			if(s==null)s=new Sprite();
			s.graphics.beginFill(col);
